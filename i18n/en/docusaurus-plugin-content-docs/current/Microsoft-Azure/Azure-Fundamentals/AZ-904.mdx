...
Azure has a physical infrastructure that includes **datacenters**, **regions**, **availability zones**, and **region pairs**.  
This infrastructure is designed to provide reliable, fast, and secure cloud services to users.

<br/>
### Azure Regions

A region is a geographic area that contains one or more data centers with low-latency network connectivity.  
Every resource created in Azure must be placed in a specific region.
...
Availability Zones are physically separate data centers within a single Azure region.  
Each zone has independent power, cooling, and networking, so if one zone fails, others continue to operate.
...
Most Azure regions are paired with another region in the same geography (at least 300 miles apart).  
Main function: Resource replication to reduce the risk of major outages such as natural disasters or power failures.
...
A resource group is a container that holds related resources in Azure.  
When creating resources, users must assign them to a resource group.
...
Azure subscriptions are used for managing, billing, and scaling resources.  
Like resource groups, subscriptions help organize both resources and resource groups.
...
If a user has many subscriptions, a more efficient way is needed to manage access, policies, and compliance across them.  
Azure Management Groups form a hierarchy above subscriptions, allowing broad policy and access control.
