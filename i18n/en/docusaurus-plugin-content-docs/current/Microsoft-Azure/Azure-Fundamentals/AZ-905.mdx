...
Azure Containers are lightweight virtualization solutions to run multiple applications on a single physical or virtual 
host without managing the operating system directly. Compared to virtual machines (VMs), containers are faster, more flexible, and use fewer resources.
